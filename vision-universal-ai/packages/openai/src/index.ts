...
export class OpenAIProvider implements AIProvider {
    ...
    return content.map((part) => {
      ...
      if (part.type === "image") {
        ...
        if (typeof part.image === "string") {
          ...
          const base64 = typeof Buffer !== "undefined" ? Buffer.from(uint8).toString("base64") : btoa(String.fromCharCode(...uint8));
  ...
  protected handleError(statusCode: number, errorData: any, model: string): never {
    const message = errorData?.error?.message || errorData?.message || `OpenAI API Error (HTTP ${statusCode})`;
