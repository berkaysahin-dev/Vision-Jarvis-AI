...
export class AnthropicProvider implements AIProvider {
    ...
    return content.map((part) => {
      ...
      if (part.type === "image") {
        ...
        if (typeof part.image === "string") {
          ...
          base64Data = typeof Buffer !== "undefined" ? Buffer.from(uint8).toString("base64") : btoa(String.fromCharCode(...uint8));
        }
  ...
  private handleError(statusCode: number, errorData: any, model: string): never {
    const message = errorData?.error?.message || errorData?.message || `Anthropic API Error (HTTP ${statusCode})`;
